...
/*   Updated: 2019/09/11 19:50:29 by vrichese         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_assets		*get_assets(SDL_Renderer *renderer)
{
	...
	if (!(assets = (t_assets *)malloc(sizeof(t_assets))))
		error(errno);
	assets->node = get_file_texture(renderer, "source/assets/node.bmp");
	assets->start = get_file_texture(renderer, "source/assets/start.bmp");
	assets->end = get_file_texture(renderer, "source/assets/end.bmp");
	assets->closed = get_file_texture(renderer, "source/assets/closed.bmp");
	assets->ant = get_file_texture(renderer, "source/assets/ant.bmp");
	assets->pipe = get_file_texture(renderer, "source/assets/pipe.bmp");
	assets->path = get_file_texture(renderer, "source/assets/path.bmp");
	if (TTF_Init())
		error_vis(TTF_GetError());
	if (!(assets->font = TTF_OpenFont("source/assets/OpenSans-Semibold.ttf", 72)))
		error_vis(TTF_GetError());
	return (assets);
}
